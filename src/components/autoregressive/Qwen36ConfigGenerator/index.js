...
commandRule: (value) =>
value === "enabled" ? "--reasoning-parser qwen3" : null,
},
toolcall: {
name: "toolcall",
...
commandRule: (value) =>
value === "enabled" ? "--tool-call-parser qwen3_coder" : null,
},
speculative: {
name: "speculative",
...
const isAmdGpu =
values.hardware === "mi300x" ||
values.hardware === "mi325x" ||
values.hardware === "mi355x";

// MambaCache is not supported on AMD GPUs
if (isAmdGpu) {
return [
{ id: "disabled", label: "Disabled", default: true, disabled: true },
];
}
...
commandRule: (value) =>
value === "v2" ? "--mamba-scheduler-strategy extra_buffer" : null,
},
},
...
const isNvidia =
hardware === "h100" || hardware === "h200" || hardware === "b200";
...
// Prepend env var if MTP is enabled on NVIDIA
...
const actualMambaCache =
speculative === "enabled" ? "v2" : values.mambaCache;
...
// Skip mambaCache command rules for AMD GPUs (not supported)
if (key === "mambaCache" && !isNvidia) return;
...
// Add speculative decoding flags for AMD (after main command)
if (speculative === "enabled" && !isNvidia) {
cmd += ` \\
