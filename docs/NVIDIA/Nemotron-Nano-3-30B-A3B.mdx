...
import NemotronConfigGenerator from '@site/src/components/NemotronConfigGenerator';
...
{/* const CONFIG = {
  ...
  generateCommand(values) {
    ...
    // We ignore the requested TP and always emit --tp 1.
    const effectiveTp = '1';

    const lines = [];
    lines.push('python3 -m sglang.launch_server \\');
    lines.push(`  --model-path ${modelName} \\`);
    lines.push('  --trust-remote-code \\');
    lines.push(`  --tp ${effectiveTp} \\`);
    lines.push(`  --kv-cache-dtype ${kvcache} \\`);

    // B200 uses FlashInfer backend; H200 can use the default attention backend
    if (hardware === 'b200') {
      lines.push('  --attention-backend flashinfer \\');
    }

    lines.push(`  --host ${host || '0.0.0.0'} \\`);
    lines.push(`  --port ${port || '30000'}`);
