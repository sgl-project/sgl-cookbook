...
function updateQuantOptions() {
    ...
    // H100 only supports FP8 for GLM-4.6
    if (hardware === 'h100') {
        bf16Radio.disabled = true;
...
- **Memory Management**: Set lower `--context-length` to conserve memory. A value of `128000` is sufficient for most scenarios, down from the default 200K.
- **Data Parallel (DP)**: Enable with `--dp 4 --enable-dp-attention` for production throughput. DP attention mechanism avoids KV cache duplication.
- **KV Cache**: Use `--kv-cache-dtype fp8_e4m3` to reduce memory by 50% (requires CUDA 11.8+).
- **Speculative Decoding**: Use EAGLE for lower latency:
  ...
  - `--speculative-draft-model-path`: Path to draft model weights (local folder or HuggingFace repo ID)
